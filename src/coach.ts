...
export class ChessCoach {
   ...
   */
  private convertMovesToNotation(moves: Move[]): string[] {
   ...
   */
  private detectOpening(moveSequence: string[]): OpeningInfo | null {
   ...
   */
  private matchesOpening(moveSequence: string[], openingMoves: string[]): boolean {
   ...
   */
  private getPlayerOpeningType(moves: Move[], color: Color): string {
    ...
    if (notation === 'e2e4') return 'King\'s Pawn (e4)';
    if (notation === 'd2d4') return 'Queen\'s Pawn (d4)';
    if (notation === 'c2c4') return 'English (c4)';
    if (notation === 'g1f3') return 'Reti (Nf3)';
    if (notation === 'e7e5' && playerMoves.length === 1) return 'Open Game (e5)';
    if (notation === 'c7c5' && playerMoves.length === 1) return 'Sicilian Defense';
    if (notation === 'e7e6' && playerMoves.length === 1) return 'French Defense';
    if (notation === 'c7c6' && playerMoves.length === 1) return 'Caro-Kann';
    if (notation === 'g8f6' && playerMoves.length === 1) return 'Indian Defense';
    if (notation === 'd7d5' && playerMoves.length === 1) return 'Closed Game (d5)';
   ...
   */
  private getStartingGuidance(): CoachAnalysis {
    return {
   ...
   */
  private getGeneralSuggestions(moveHistory: Move[]): string[] {
   ...
   */
  private getGeneralTips(moveCount: number): string[] {
    if (moveCount < 6) {
      return [
